...
describe('RetryEngine', () => {
  ...
  describe('Retry Logic', () => {
    test('should respect maximum retry limit', async () => {
      ...
      await expect(retryEngine.start(mockOperation)).rejects.toThrow(RetryExhaustedError);
    ...
    test('should respect cap delay limit', async () => {
      ...
      retryEngineWithCap.retryCount = 5; // This would normally give 32000ms
      const delay = await retryEngineWithCap.calculateDelay();

      expect(delay).toBe(2000); // Should be capped
    });
  });
  ...
  describe('Metrics and Monitoring', () => {
    ...
    test('should limit connection history size', () => {
      ...
      for (let i = 0; i < 25; i++) {
        retryEngine.updateConnectionHistory(i % 2 === 0, new Error(`Error ${i}`));
      }
  ...
  describe('Callback Functions', () => {
    ...
    test('should call onFailure callback when max retries exceeded', async () => {
      ...
      await expect(retryEngineWithCallbacks.start(mockOperation)).rejects.toThrow();
  ...
  describe('Dynamic Configuration', () => {
    ...
    test('should fallback to standard delay calculation on dynamic config error', async () => {
      const mockDynamicConfig = {
        calculateRetryDelays: jest.fn().mockRejectedValue(new Error('Config error'))
      };
  ...
  describe('Memory-based Delay Adjustment', () => {
    test('should increase delay under high memory usage', async () => {
      ...
      const expectedDelay = retryEngine.baseDelay * 2; // Should double due to high memory
